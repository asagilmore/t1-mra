...
class PerceptualLoss_3d():
    ...
    def _get_2d_loss(self, output, target):
        rgb_output = output.expand(-1, 3, -1, -1)
        rgb_target = target.expand(-1, 3, -1, -1)
        loss = self.PerceptualLoss.get_loss(output, target)
        return loss
    
    def _get_loss_of_volume(self, input, mask, axis):
        running_loss = 0.0
        output_scan = []
        for index in range(input.shape[axis]):
            # Slice the scan based on the axis
            if axis == 1:
                input_slice = input[:, index, :, :]
                mask_slice = mask[:, index, :, :]
            elif axis == 2:
                scan_slice = input[:, :, index, :]
                mask_slice = mask[:, :, index, :]
            elif axis == 3:
                scan_slice = input[:, :, :, index]
                mask_slice = mask[:, :, :, index]
            ...
            loss = self._get_2d_loss(scan_slice, mask_slice)
            running_loss += loss
